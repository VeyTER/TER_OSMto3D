...
public class Triangle
{
	...
	public void calculCentre()
	{
		...
		/*axc = (Math.Pow(nclon, 2) - Math.Pow(nblon, 2) + Math.Pow(nclat, 2) - Math.Pow(nblat, 2)) / (2 * (nclat - nblat));
        bxc = (Math.Pow(nblon, 2) - Math.Pow(nalon, 2) + Math.Pow(nblat, 2) - Math.Pow(nalat, 2)) / (2 * (nblat - nalat));
        dxc = ((nblon - nalon) / (nblat - nalat)) - ((nclon - nblon) / (nclat - nclat));
        xc = (axc - bxc) / dxc;
        */
